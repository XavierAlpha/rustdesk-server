...
fn parse_downgrade_start(value: &str) -> ResultType<usize> {
    ...
    seconds
        .checked_mul(1_000)
        .ok_or_else(|| hbb_common::anyhow::anyhow!("DOWNGRADE_START_CHECK is too large"))
}
...
fn get_server_sk(key: &str) -> ResultType<String> {
    ...
    let decoded = BASE64
        .decode(key)
        .map_err(|_| hbb_common::anyhow::anyhow!("The relay key must be base64 encoded"))?;
    let public_key = match decoded.len() {
        sign::PUBLICKEYBYTES => sign::PublicKey::from_slice(&decoded)
            .ok_or_else(|| hbb_common::anyhow::anyhow!("The relay public key is invalid"))?,
        sign::SECRETKEYBYTES => {
