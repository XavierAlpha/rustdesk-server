...
DEBIAN_HEADER_RE = re.compile(r"^rustdesk-server \(([^)]+)\) [^;]+; urgency=[a-z]+$")
...
def _manifest_version(root: Path) -> str:
    ...
    if not isinstance(package, dict) or package.get("name") != "hbbs":
        raise MetadataError("Cargo.toml must describe package hbbs")
...
def _lock_version(root: Path) -> str:
    ...
    matches = [
        ...
        if isinstance(package, dict) and package.get("name") == "hbbs"
    ]
    if len(matches) != 1 or not isinstance(matches[0].get("version"), str):
        raise MetadataError("Cargo.lock must contain exactly one hbbs package")
    return matches[0]["version"]


def _debian_version(root: Path) -> str:
    path = root / "debian" / "changelog"
    try:
        first_line = path.read_text(encoding="utf-8").splitlines()[0]
    except (OSError, IndexError) as error:
        raise MetadataError(f"cannot read {path}: {error}") from error
    match = DEBIAN_HEADER_RE.fullmatch(first_line)
    if match is None:
        raise MetadataError("debian/changelog has an invalid first entry")
...
def load_metadata(root: Path) -> ReleaseMetadata:
    ...
    debian_version = _debian_version(root)
    if lock_version != manifest_version or debian_version != manifest_version:
        raise MetadataError(
            "version mismatch: "
            f"Cargo.toml={manifest_version}, Cargo.lock={lock_version}, debian/changelog={debian_version}"
        )
